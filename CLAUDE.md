...
4. If `CLAUDE.md` was modified, prompts the developer via `/dev/tty` to amend the last commit (`y`), skip (`skip`), or abort the push (`N`)
...
- stdin is redirected from `/dev/tty` before the interactive prompt because git owns stdin during hook execution
